...
class VectorStore:
    ...
    def add_documents(self, documents):
        ...
        for i, doc in enumerate(documents):
            ...
            # 兼容不同 scipy 版本（csr_array / coo_array）
            if hasattr(sparse_row, 'col'):
                indices = sparse_row.col
            elif hasattr(sparse_row, 'indices'):
                indices = sparse_row.indices
            else:
                indices = sparse_row.tocoo().col
    ...
    def hybrid_search_with_rerank(self, query, k=conf.RETRIEVAL_K, source_filter=None):
        ...
        # 兼容不同 scipy 版本
        if hasattr(sparse_row, 'col'):
            sparse_indices = sparse_row.col
        elif hasattr(sparse_row, 'indices'):
            sparse_indices = sparse_row.indices
        else:
            sparse_indices = sparse_row.tocoo().col
        sparse_values = sparse_row.data
        # 将索引和值配对，填充稀疏向量字典
