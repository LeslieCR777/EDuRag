...
class RAGSystem:
    ...
    def __init__(self, vector_store, llm, llm_stream=None):
        ...
        #   设置大语言模型调用函数（非流式，用于检索）
        self.llm = llm
        #   设置大语言模型流式调用函数（用于生成回答）
        self.llm_stream = llm_stream or llm
        #   获取 RAG 提示模板
        self.rag_prompt_template = RAGPrompts.rag_prompt()
        #   初始化查询分类器
    ...
    def generate_answer(self, query, source_filter=None, history=None):
        ...
        prompt_input = self.rag_prompt_template.format(context=context,
                                                       question=query,
                                                       history=history_context,
                                                       phone=conf.CUSTOMER_SERVICE_PHONE)
        try:
            for token in self.llm_stream(prompt_input):
                yield token
