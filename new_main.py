...
class IntegratedQASystem:
    def __init__(self):
        ...
        # 初始化 RAG 系统，传入向量存储和 Ollama 调用函数（流式+非流式）
        self.rag_system = RAGSystem(self.vector_store, self.call_ollama, self.call_ollama_stream)
        # 初始化对话历史表，用于存储会话记录
    ...
    def call_ollama(self, prompt):
        """调用本地 Ollama 模型（非流式，用于策略选择/回溯检索）"""
        try:
    ...
    def call_ollama_stream(self, prompt):
        ...
        try:
            ...
            self.logger.error(f"Ollama 流式调用失败: {e}")
